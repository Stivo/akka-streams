...
public class RxjavaTest {
    public static void main(String[] args) throws Exception {
        ...
        backup.mkdir();
        List<Path> toBackup = Files.walk(Paths.get("restored"))
                .filter(Files::isRegularFile)
                .filter(e -> !e.toAbsolutePath().toString().contains(".git"))
                .collect(Collectors.toList());
        CompletableFuture<Boolean> done = new CompletableFuture<>();
//        toBackup.parallelStream()
//                .flatMap(e -> Stream.of(backupFile(e.toFile())))
//                .forEach(e -> e.join());
        Flowable.fromIterable(toBackup)
                .parallel(10)
                .map(e -> backupFile(e.toFile()))
                .sequential().forEach(e -> e.join());
//        booleanParallelFlowable.sequential().forEach(e -> )
//                .forEach(e -> e.join());
        BackupStreams.shutdown();
    }

    private static CompletableFuture<Boolean> backupFile(File file) {
        ...
        return future1.thenCompose(f -> future2);
    }
